...
HRESULT present_hooked(IDXGISwapChain* swapChain, UINT syncInterval, UINT flags)
{
	...
	if (!device)
	{
		...
		ImGui::GetIO().Fonts->AddFontFromFileTTF(e("C:\\Windows\\Fonts\\calibri.ttf"), 13.0f);
...
bool DummDeviceGetPresent()
{
	...
	RegisterClassExA(&wc);
	hWnd1 = CreateWindowA("a", NULL, WS_OVERLAPPEDWINDOW, 0, 0, 1280, 1024, NULL, NULL, wc.hInstance, NULL);
